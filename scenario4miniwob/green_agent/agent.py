...
from tools import TOOL_FUNCTIONS   # You will define this as a dict in tools.py
...
def load_agent_card_toml(agent_name: str):
    """
    Load <agent_name>.toml located in the same directory as this file.
    """
...
class SimpleGreenAgentExecutor(AgentExecutor):
    def __init__(self):
        pass
...
def start_green_agent(agent_name="green_agent_card", host="localhost", port=9115):
    """
    This function starts the agent HTTP server.
    It will be called by run.sh using Python 3.11.
    """
    print("Starting green agent...")

    # Load the agent card
    agent_card_dict = load_agent_card_toml(agent_name)
    agent_card_dict["url"] = f"http://{host}:{port}"  # required by A2A
    agent_card = AgentCard(**agent_card_dict)

    # Handler
    request_handler = DefaultRequestHandler(
        agent_executor=SimpleGreenAgentExecutor(),
